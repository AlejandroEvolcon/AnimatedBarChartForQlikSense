define(["jquery", "text!./animatedbarchart.css","./d3.min"], function($, cssContent) {'use strict';
	...
	return {
		...
		paint : function($element,layout) {
          ...
          if(document.getElementById(id)){
            //if it has been created, empty its contentsso we can redraw it
            $("#" + id).empty();
          }
          else{
...
var viz = function(data,dimensionLabels,measureLabels,width,height,id){
    ...
    x.domain(names);
    y.domain([0, d3.max(data, function(d) { return d.Metric1; })]).nice();
